...
def SIFRank(text, SIF, en_model, method="average", N=15,
    ...
    keywords = heuristics.apply_heuristics(text, dist_sorted)
    return keywords[0:N]
...
def SIFRank_plus(text, SIF, en_model, method="average", N=15,
    ...
    for k in keywords:
        if len(dict_kw) < N:
            lemmatised = " ".join([lemmatiser.lemmatize(w) for w in k[0].split()])
            if lemmatised not in dict_kw:
                dict_kw[lemmatised] = k[1]
    return dict_kw
